...
def elemLoop(x,u,elem,masterElement,material_law,mat,flag_tangent,b,K,fint,fvol,ndm,ndf,tdm,nel,nen,nqp,dt):
    ...
    for e in range(nel):
        ...
        for q in range(nqp):
            ...
            for A in range(nen):
                ...
                # Loop over the number of nodes B
                for B in range (nen):
                    
                    # Gradient of the shape functions of node B
                    G_B = G[B,:]
                    
                    # Calculate KAB
                    KAB = np.zeros((ndm,ndm))
                    for m in range (ndm):
                        for n in range (ndm):
                            for o in range (ndm):
                                for p in range (ndm):
                                    KAB[n,o] += G_A[m]*C4[m,n,o,p]*G_B[p]*dV
                    
                    # Assemble KAB into Ke
                    for i in range(ndm):
                        for j in range(ndm):
                            Ke[A*ndf+i,B*ndf+j] += KAB[i,j]
        ...
        for i, gdof_i in enumerate(gdof):
            ...
            for j,gdof_j in enumerate(gdof):
                K[gdof_i,gdof_j] += Ke[i,j]
        
        
    return K, fint, fvol, elem